...
! Scaling density !
roverb1 = rhomax2/b_max1
roverb2 = rhomax2/b_max2
rbonethird1 = roverb1**(1.0D0/3.0D0)
rbonethird2 = roverb2**(1.0D0/3.0D0)
...
! DM Atmospheric enthalpy !
hhata1 = (ha1/potential)

! NM Atmospheric enthalpy !
hhata2 = (ha2/potential)
hhatfloor2 = (hfloor2/potential)
	...
	DO i = 1, KDIV
		DO j = 1, NDIV
			...
			ELSEIF(hhatnew2(i,j) == hhata2) THEN
				rho2(i,j) = rhoa2
			ELSE
				rho2(i,j) = ((b_max2*hhatnew2(i,j)*potential/8.0D0/a_max2)**2 - 1.0D0)**3
				IF(rho2(i,j) < 0.0D0) THEN
					rho2(i,j) = 0.0D0
				END IF
				rho2(i,j) = SQRT(rho2(i,j))/roverb2
			END IF
			dlfmmo2 (i,j) = (rho2(i,j))**(1.0D0/3.0D0)*rbonethird2
		END DO	
	END DO
ELSEIF(yex_flag == 1) THEN
	DO i = 1, KDIV
		DO j = 1, NDIV
			...
			ELSEIF(hhatnew2(i,j) > hhata2) THEN
				rho2(i,j) = (((b_max2*hhatnew2(i,j)*potential/8.0D0/a_max2) + 1.0D0)**2 - 1.0D0)**3
				IF(rho2(i,j) < 0.0D0) THEN
					rho2(i,j) = 0.0D0
				END IF
				rho2(i,j) = SQRT(rho2(i,j))/roverb2
	...
	DO i = 1, KDIV
		DO j = 1, NDIV
			...
			ELSEIF(hhatnew1(i,j) == hhata1) THEN
				rho1(i,j) = rhoa1
			ELSE
				rho1 (i,j)= ((b_max1*hhatnew1(i,j)*potential/8.0D0/a_max1)**2 - 1.0D0)**3
				IF(rho1(i,j) < 0.0D0) THEN
					rho1(i,j) = 0.0D0
				END IF
				rho1(i,j) = SQRT(rho1(i,j))/roverb1
			END IF
			dlfmmo1 (i,j) = (rho1(i,j))**(1.0D0/3.0D0)*rbonethird1
		END DO
	END DO
